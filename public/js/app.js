...
// async function fetchProducts() {
//     try {
//         const response = await fetch('/products'); // removed sellerId filter
...
//                 <div class="card-body d-flex flex-column">
//                     <p class="text-muted mb-1">Seller: ${product.sellerName || "Unknown"}</p>
//                     <h6 class="card-title">${product.name}</h6>
//                     <p class="price">₱${product.price}</p>
...
function updateCartCount() {
    const totalItems = cart.reduce((sum, item) => sum + item.quantity, 0);
    cartCount.textContent = totalItems;
    if (totalItems > 0) document.getElementById("cartNav").classList.remove("d-none");
    else document.getElementById("cartNav").classList.add("d-none");
...
function renderCartDropdown() {
    ...
    cart.forEach(item => {
        ...
        li.innerHTML = `
            <div class="d-flex align-items-center">
                <img src="${item.image}" alt="${item.name}" class="cart-thumb me-2" style="width:40px; height:40px; object-fit:cover;">
                <div>
                    <strong>${item.name}</strong><br>
                    <small>₱${item.price} x ${item.quantity}</small>
                </div>
...
async function fetchProducts() {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.error("Failed to fetch products:", errData);
            return [];
        }
