...
//             showMessage("Editing product. Update and click Save.", "info");
//         });
// }
...
// //const showMessage = document.getElementById("productMessage");
...
//     let price = document.getElementById('productPrice').value.replace("$", "").trim();
//     const imageFile = document.getElementById('ProductAvatar').files[0];
...
//         showMessage("Please enter a valid price greater than 0.", "danger");
//         return;
//     }

//     if (!imageFile) {
//         showMessage("Please upload a product image.", "danger");
//         return;
//     }
...
//         showMessage("Product successfully added!", "success");
...
function applyLoginUI() {
  ...
  document.getElementById("sellerNameDisplay").textContent = currentUser.firstName;

  const profilePic = document.getElementById("profilePicDisplay");
  if (profilePic) {
    profilePic.src = currentUser.avatar || "./images/default-avatar.png";
    profilePic.onerror = () => {
      profilePic.src = "./images/default-avatar.png";
    };
  }
      ...
      li.innerHTML = `
        <div class="d-flex align-items-center">
          <img src="${prod.image}" alt="${prod.name}" 
               style="width:60px; height:60px; object-fit:cover; margin-right:10px; border:1px solid #ccc">
          <div>
            <strong>${prod.name}</strong><br>
            ${prod.price}
          </div>
...
function editProduct(id) {
  fetch(`/products?sellerId=${currentUser.id}`)
    ...
    .then(products => {
      ...
      document.getElementById("productName").value = product.name;
      document.getElementById("productPrice").value = product.price;
      avatarPreview.src = product.image;
...
productForm?.addEventListener("submit", async e => {
  ...
  try {
    ...
    productForm.reset();
    avatarPreview.src = "./images/box.jpg";
    loadProducts();
